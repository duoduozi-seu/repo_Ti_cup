...
///* DuoDuoZi modified 2025.7.29 19:53 *///
...
bool check_yaw_change(float current_yaw, float threshold_deg) {
    // 1. 计算直接角度差值
    float direct_diff = fabs(current_yaw - prev_yaw);
    
    // 2. 计算跨边界角度差值
    float cross_diff = 360.0f - direct_diff;
    
    // 3. 确定实际最小角度变化
    float actual_diff = (direct_diff < cross_diff) ? direct_diff : cross_diff;
    
    // 4. 检查是否达到阈值
    return actual_diff >= threshold_deg;
}
